...
Generate, cut and preview one sheet with `python3 tools/icon_batch.py cozy-paper C1` (Azure); `--all` for the set.
...
```
A 3x3 grid sprite sheet of 9 separate user interface icons for a cozy storybook mobile game.
...
Style: soft, friendly UI glyphs for a cozy storybook game. Each icon is ONE SOLID FILLED shape in pure flat white (#FFFFFF) with very rounded, slightly hand-drawn, pillowy edges — gentle and warm, never sharp. Never line art, never outlined, never thin, no gradients, no shading, no 3D. Every icon is the same visual weight and the same optical size, centred in its own ninth with clear black space around it.
