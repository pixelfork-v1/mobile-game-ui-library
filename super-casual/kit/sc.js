...
(function () {
  ...
  function markLong(root = document) {
    ...
    root.querySelectorAll?.('.sc-action > .sc-text').forEach(span => {
      ...
      span.style.setProperty('--fs', (size * Math.max(.55, room / natural)).toFixed(1) + 'px');
    });
  }
  ...
  function upgradeJoystick(el) {
    ...
    if (!el.querySelector(':scope > .sc-joystick-knob')) {
      el.innerHTML = (dpad ? '<span class="sc-joystick-cross"></span><span class="sc-joystick-press"></span>' : '') +
        ['up', 'right', 'down', 'left'].map(d => `<span class="sc-joystick-arrow" data-dir="${d}"></span>`).join('') +
        '<span class="sc-joystick-knob"></span>';
    } else if (dpad && !el.querySelector(':scope > .sc-joystick-cross')) {
      el.insertAdjacentHTML('afterbegin', '<span class="sc-joystick-cross"></span><span class="sc-joystick-press"></span>');
    }
  ...
  function renderJoystick(el) {
    ...
    el.querySelectorAll(':scope > .sc-joystick-arrow').forEach(a => a.classList.toggle('sc-on', !!st.dir && st.dir.includes(a.dataset.dir)));
    if (el.dataset.variant === 'dpad') { el.style.setProperty('--px', (50 + st.x * 30) + '%'); el.style.setProperty('--py', (50 + st.y * 30) + '%'); }
  }
